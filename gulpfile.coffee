...
gulp.task 'sign:osx64', ['build:osx64'], ->
  shelljs.exec 'codesign -v -f -s "Alexandru Rosianu Apps" ./build/nwjsupdatetest/osx64/Starter.app/Contents/Frameworks/*'
  shelljs.exec 'codesign -v -f -s "Alexandru Rosianu Apps" ./build/nwjsupdatetest/osx64/Starter.app'
  shelljs.exec 'codesign -v --display ./build/nwjsupdatetest/osx64/Starter.app'
  shelljs.exec 'codesign -v --verify ./build/nwjsupdatetest/osx64/Starter.app'
...
gulp.task 'pack:osx64', ['sign:osx64'], ->
  ...
  shelljs.rm '-f', './dist/nwjsupdatetest.dmg'   # appdmg fails if the dmg already exists

  gulp.src []
    .pipe require('gulp-appdmg')
      source: './assets-osx/dmg.json'
      target: './dist/nwjsupdatetest.dmg'
